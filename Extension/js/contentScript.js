...
function settingTreeview() {
    ...
    for (let i = 0; i < uidata.length; i++) {
        ...
        if (data == true && uidata[i].querySelector("ul") != null) {
            icon.innerHTML = `<img src="${chrome.runtime.getURL('images/arrow-right.png')}" class="plus_icon_js"  />`
            uidata[i].append(icon);
        }
        else {
            icon.innerHTML = `<img src="${chrome.runtime.getURL('images/up-arrow.png')}" class="width100"  />`
            uidata[i].append(icon);
